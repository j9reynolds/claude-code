...
PAIR = pu.city,ST | del.city,ST, + flags. OTP = actual_arrival <= COALESCE(sched_arrive_late,
sched_arrive_early); OTD same on delivery stop; both void→'V'. DISPATCH ⚠️ SYNTHETIC: "actual
dispatch" = rate-con posted_date (order_post_hist posted_type='C') MINUS random 55-67 min via
NEWID() → always on-time (~100%) and non-deterministic; not a real metric — flagged to fix
(use a real departure ts). Reason codes added by hand. Overview aggregates the Y/N flags.
