...
PAIR = pu.city,ST | del.city,ST, + flags. ⚠️ ALL THREE actuals RANDOMIZED (the version actually used — confirmed: both May & Aug reports
label voids "Order is VOID", which only this variant emits; an earlier variant Justin also has
uses 'V'/'N'/'V' and does NOT randomize arrival/delivery — NOT the one in use). Each actual =
real McLeod value MINUS random 55-67 min via NEWID(): RandomArrival (pu.actual_arrival)→OTP,
RandomDelivery (del.actual_arrival)→OTD, ROPH (rate-con posted_date, order_post_hist
posted_type='C')→Dispatch. Effect: OTP/OTD/Dispatch all scored vs a time ~1h earlier than
recorded → systematically better-than-actual, non-deterministic (changes each run), doesn't
reconcile to McLeod on audit; Dispatch ~100% by construction. FLAGGED to Justin: compute
on-time from real actual_arrival/actual_departure. Void→"Order is VOID" all 3 cols. Reason
codes added by hand. Overview aggregates the Y/N flags.
