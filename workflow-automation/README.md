...
| `reporting/` | **Recurring reports (#6):** the real **USPS GEGW "Self Report"** Justin files monthly to J.B. Hunt (tender 0029H) — generator (`report_usps_self_report.py`, 10/10 tests) + DGLIQ query, reproducing the `.xlsb` layout exactly (Lane · Load Count · OTP · OT Dispatch · OTD · Comments, per-lane + TOTAL). Plus a reusable monthly customer-performance report (`report_customer_monthly.py`) and the framework spec. Delivery gated on Microsoft 365 re-auth. |
