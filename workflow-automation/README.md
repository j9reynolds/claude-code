...
| `reporting/` | **Recurring reports (#6):** the real **USPS "Self Report"** Justin files monthly to J.B. Hunt (tender 0029H) — generator (`report_usps_self_report.py`, 10/10 tests) that consumes the **J.B. Hunt raw-data extract** and reproduces the filed report **exactly** (verified against the real May 2026 report: 27/27 lanes, 0 differences): Lane · Load Count · OTP · OT Dispatch · OTD · Comments, single-% cells, per-lane + TOTAL. `report_usps_self_report.sql` is an optional McLeod cross-check (not the source). Plus a reusable monthly customer-performance report (`report_customer_monthly.py`) and the framework spec. Delivery gated on Microsoft 365 re-auth. |
