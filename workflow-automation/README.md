...
| `reporting/` | **Recurring reports (#6):** the real **USPS "Self Report"** Justin files monthly to J.B. Hunt (tender 0029H). End-to-end **pipeline** `usps_selfreport_pipeline.py` (stdlib-only — reads/writes `.xlsx` as zipped XML, no installs): **extract** (`usps_selfreport_extract.sql`, Delta's real McLeod LME query, run in SSMS → CSV) → **generate** (`report_usps_self_report.py`) → **fill** an Overview `.xlsx` (Lane · Load Count · OTP · OT Dispatch · OTD · Comments, single-% cells, per-lane + TOTAL). Verified end-to-end against the real May 2026 report: **27/27 lanes, 0 differences**. Tests 8+10. (Flagged for fix: the extract's "dispatch" metric is synthetic — rate-con time minus a random hour — so it's ~100% by construction.) Plus a reusable monthly customer-performance report (`report_customer_monthly.py`) and the framework spec. Delivery gated on Microsoft 365 re-auth. |
