...
| `mailbox-hygiene/` | **Exchange Online duplicate removal:** `Remove-DuplicateMessages.ps1` finds mail that exists more than once in a mailbox, keeps exactly one copy, and reports, quarantines, or deletes the rest. Report-only by default. Groups on `internetMessageId` (the identity that survives a folder move - Graph's per-message `id` does not) or on a content hash for re-sent copies; `PerFolder` scope by default so a deliberately filed copy is never collapsed. Runs on Microsoft Graph because `Search-Mailbox` is retired and Compliance Search purge cannot keep one copy; HTTPS/443 only, so it works where basic-auth IMAP does not. Removal is staged Report -> Quarantine -> Delete -> Purge, every destructive mode opt-in and `-WhatIf`-able. 19 unit tests over the keep/remove logic, no mailbox needed. |
