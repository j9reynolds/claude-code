...
| `reporting/` | **Recurring reports (#6):** the real **USPS "Self Report"** Justin files monthly to J.B. Hunt (tender 0029H). End-to-end **pipeline** `usps_selfreport_pipeline.py` (stdlib-only — reads/writes `.xlsx` as zipped XML, no installs): **extract** the J.B. Hunt raw tab → **generate** (`report_usps_self_report.py`) → **fill** an Overview `.xlsx` (Lane · Load Count · OTP · OT Dispatch · OTD · Comments, single-% cells, per-lane + TOTAL). Verified end-to-end against the real May 2026 report: **27/27 lanes, 0 differences**. Tests 6+10. `report_usps_self_report.sql` is an optional McLeod cross-check (not the source). Plus a reusable monthly customer-performance report (`report_customer_monthly.py`) and the framework spec. Delivery gated on Microsoft 365 re-auth. |
