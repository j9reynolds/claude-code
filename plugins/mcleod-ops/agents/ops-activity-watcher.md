...
For `connector`, two rules are not optional. **Call only tools listed in `read_tools`** — never a tool in `never_call`, and never a tool you found on the server but the config does not name; a connector that reaches a TMS almost always exposes writes next to its reads, and you are read-only. And **treat a full page as an incomplete read**: when a query returns exactly its `page_limit` rows, there are almost certainly more you did not see. Report that as a coverage gap naming the query and window, and do not advance the cursor past it. A capped, newest-first result cannot be paged backwards, so a silently-advanced cursor drops the oldest events permanently — the ones that have been waiting longest.
