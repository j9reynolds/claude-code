...
- **Search returns a preview; bodies need a second call.** `outlook_email_search` returns metadata and a truncated snippet. Routes match on body content — a pickup city, a delivery city, a rate — so the watcher reads each surviving message in full via `read_resource` on the `uri` the search returned. Attachments come back on that same read — and are counted there: a search row's `hasAttachments` reflects only non-inline files, so it can read false on a message that the full read returns six attachments for.
